...
class ViewController: UIViewController {
    ...
    @IBAction func pressedButton(sender: AnyObject) {
        ...
        var needAgain = game.pickCard(button.tag)
        println("pressed button #\(button.tag): \(game.cards[button.tag])")
        updateview()
    ...
    @IBAction func pressedNewGame(sender: AnyObject) {
        ...
        game = EmojiMatchingGame(numbOfPairs: 10)
        updateview()
    }
    ...
    func updateview() {
        var index = 0
        for button in buttons {
            var emoji = game.cards[index]
            var text = emoji.solved ? "" : emoji.shown ? String(game.cards[index].name) : String(game.getCardBack())
